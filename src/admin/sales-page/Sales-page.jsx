...
import { Badge, Button, Card, message, Modal, Table, Tabs } from "antd";
...
const SalesPage = () => {
  ...
  const currentBillDataMemo = useMemo(
    () => items.find((item) => item.key === currentBill) || {},
    [items, currentBill],
  );
  // e cong soket
  ...
  const updatePayInfo = ({
    ...
    setItems((prevItems) =>
      prevItems.map((item) => {
        ...
        const cashCustomerMoney =
          cash !== null ? cash : prevPayInfo.cashCustomerMoney || 0;
        const bankCustomerMoney =
          bank !== null ? bank : prevPayInfo.bankCustomerMoney || 0;

        const customerMoney =
          Number(cashCustomerMoney) + Number(bankCustomerMoney);
        ...
        return {
          ...
          payInfo: {
            ...
            customerMoney,
            transactionCode:
              transactionCode ?? prevPayInfo.transactionCode ?? null,
  ...
  useEffect(() => {
    setItems((prevItems) => {
      ...
      const isSame = prevItems.every(
        (item, index) => item.canPayment === newItems[index].canPayment,
      );
      ...
      const discount =
        voucher.discountType === "PERCENT"
          ? Math.min(
              ...
              voucher.discountMaxValue || Infinity,
            )
    ...
    vouchers.forEach((voucher) => {
      if (totalAmount < (voucher.billMinValue || 0)) {
        ...
        const potentialDiscount =
          voucher.discountType === "PERCENT"
            ? Math.min(
                (voucher.billMinValue * voucher.discountValue) / 100,
                voucher.discountMaxValue || Infinity,
              )
            : voucher.discountValue;
    ...
    setItems((prevItems) =>
      prevItems.map((item) => {
        if (item.key === currentBill) {
          ...
          const existingProduct = item.productList.find(
            (p) => p.id === product.id && p.price === product.price,
          );
          ...
          if (existingProduct) {
            if (isDeleteProduct) {
              ...
              updatedProductList = item.productList.filter(
                (p) => !(p.id === product.id && p.price === product.price),
              );
            } else {
              if (product.price !== existingProduct.price) {
                ...
                updatedProductList = [
                  ...item.productList.filter(
                    (p) =>
                      !(
                        p.id === product.id && p.price === existingProduct.price
                      ),
                  ),
                  {
                    ...existingProduct,
                    quantityInCart: existingProduct.quantityInCart,
                  }, // Giữ sản phẩm cũ với giá cũ
                  {
                    ...product,
                    quantityInCart: quantityChange,
                    key: `${product.id}-${product.price}`,
                  }, // Thêm sản phẩm mới với giá mới
                ];
              } else {
                ...
                updatedProductList = item.productList.map((p) =>
                  p.id === product.id && p.price === product.price
                    ? {
                        ...p,
                        quantityInCart: isRestore
                          ? p.quantityInCart - quantityChange
                          : p.quantityInCart + quantityChange,
                      }
                    : p,
                );
          ...
          const newTotalAmount = updatedProductList.reduce((total, p) => {
            ...
            const discountValue = hasPromotion
              ? p.promotionResponse.discountValue
              : 0;
          ...
          const newTotalAfterDiscount = Math.max(
            0,
          ...
          const count = updatedProductList.reduce(
            (total, p) => total + (p.quantityInCart || 0),
            0,
          );
          ...
          return {
            ...
            label: (
              <span>
                {item.itemName}{" "}
                <Badge showZero={true} className={"mb-2"} count={count} />
              </span>
            ),
            ...
            payInfo: {
              ...
              change:
                (item.payInfo?.cashCustomerMoney || 0) +
                (item.payInfo?.bankCustomerMoney || 0) -
  ...
  const handleRemoveProduct = async (product) => {
    ...
    await axiosInstance.post(
      "/api/admin/bill/restore-quantity",
      restoreQuantityPayload,
    );
    updateProductList(
      currentBill,
      product,
  ...
  const handleOnChangeQuantityProduct = async (
    product,
    quantity,
    isRestore,
  ) => {
  ...
  const onCustomerSelect = async (customer) => {
    ...
    try {
      ...
      const response = await axiosInstance.get(
        `/api/admin/bill/vouchers/${customer.id}`,
      );
  ...
  const calculateTotalAmount = (bill) => {
    return bill.productList.reduce((total, product) => {
      ...
      const discountValue = hasPromotion
        ? product.promotionResponse.discountValue
        : 0;
  ...
  const handleOnSelectedVoucher = (voucher) => {
    const originalTotal = calculateTotalAmount(
      items.find((item) => item.key === currentBill),
    );
  ...
  const handleOnPayment = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        isCashAndBank: !!(
          bill.payInfo?.cashCustomerMoney && bill.payInfo?.bankCustomerMoney
        ),
        ...
        moneyBeforeDiscount:
          (bill.payInfo?.amount || 0) + (bill.payInfo?.discount || 0),
        completeDate: null,
        confirmDate: new Date().toISOString(),
        desiredDateOfReceipt: null,
        shipDate: null,
        shippingAddressId:
          bill.addressShipping?.id ||
          bill.customerInfo?.addresses?.[0]?.id ||
          null,
        numberPhone:
          bill.recipientPhoneNumber || bill.customerInfo?.phoneNumber || "",
        ...
        billDetailRequests: bill.productList.map((product) => ({
          price:
            product.price *
            (1 - (product.promotionResponse?.discountValue || 0) / 100),
  ...
  const handleOnPrintBill = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${baseUrl}/api/admin/bill/print-bill/${id}`,
        {
          responseType: "blob",
        },
      );
  ...
  const onAddressChange = async (
    ...
    let totalFee =
      items.find((item) => item.key === currentBill).shippingFee ?? 0;
    const isCurrentWard = !!items.find(
      (item) =>
        item.key === currentBill &&
        item.detailAddressShipping.wardId === selectedWard,
    );
    ...
    setItems((prevItems) =>
      prevItems.map((item) => {
        if (item.key === currentBill) {
          ...
          const errorMsg = isValidPhone ? "" : "Số điện thoại không hợp lệ!";
  ...
  const eventProductDetailChange = (value) => {
    setItems((prevItems) =>
      prevItems.map((item) => {
        ...
        const existingProductIndex = item.productList.findIndex(
          (p) => p.id === value.id,
        );
        ...
        if (existingProductIndex !== -1) {
          ...
          updatedProductList[existingProductIndex] = {
            ...value,
            quantityInCart:
              item.productList[existingProductIndex].quantityInCart,
          };
          ...
          const discountAmount = bestVoucher ? bestVoucher.currentDiscount : 0;
  ...
  return (
    <div className={"d-flex flex-column gap-3"}>
      <Modal open={open} width={"75%"} onOk={handleOk} onCancel={handleCancel}>
        <ProductDetailModal
      ...
      <div className={"d-flex justify-content-between"}>
        <Button type={"primary"} onClick={handleOnCreateBill}>
          Tạo hóa đơn
        </Button>
      ...
      <Card>
        <div className={"d-flex justify-content-between"}>
          ...
          <div className={"d-flex gap-3"}>
            <Button type={"primary"} onClick={() => setIsScanQr(true)}>
              QR Code
            </Button>
        ...
        <Table
          dataSource={
            items.find((item) => item.key === currentBill)?.productList || []
          }
