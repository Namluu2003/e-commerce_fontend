...
const ChartStatusBill = () => {
      ...
      try {
        ...
        if (response.data && response.data.data) {
          ...
          const formattedChartData = Object.keys(apiResponse).map((key) => ({
            name:
              STATUS_LABELS[key.replace("Percent", "").toUpperCase()] ||
              key.replace("Percent", ""),
            uv: apiResponse[key],
  ...
  const handleCustomRangeChange = (dates) => {
    if (dates && dates.length === 2) {
      ...
      setChartTitle(
        `Biểu đồ trạng thái đơn hàng ${dates[0].format("DD/MM/YYYY")} - ${dates[1].format("DD/MM/YYYY")}`,
      );
    }
  };
    ...
    <Card>
      <Row gutter={[16, 16]}>
        ...
        <Col xs={24} md={14}>
          <Card
            title={
              <span style={{ color: "black", fontSize: 15 }}>{tableTitle}</span>
            }
            style={{
              background: "white",
              height: "100%",
        ...
        <Col xs={24} md={10}>
          <Card
            title={
              <span style={{ color: "balck", fontSize: 15 }}>{chartTitle}</span>
            }
            style={{
              background: "white",
              height: "100%",
